...
### G1 learned stride hero

<p align="center">
  <picture>
    <source media="(prefers-reduced-motion: reduce)" srcset="docs/media/unitree-g1-learned-stride.png">
    <img src="docs/media/unitree-g1-learned-stride.gif" alt="Side-by-side official Unitree G1 stepper and command-conditioned differential-steering stride in a robotics test bay, with measured-window trails" width="960">
  </picture>
  <br>
  <sub>The blue panel is the plain hybrid stepper at the v0.1 forward command (<code>0.0276&nbsp;m/s</code>); the orange panel uses the pinned learned torque overlay with the same speed command plus <code>+0.05&nbsp;rad/s</code> differential steering. Both panels discard the same 8&nbsp;s transient before the commanded walker clears <code>0.196&nbsp;m</code> and <code>0.235&nbsp;m</code> in its measured windows while staying at full height. The real wgpu capture is staged in a render-only robotics test bay with floor panels, safety markings, an equipment wall, and overhead fixtures; example 63 generates it, while the headless smoke path remains part of CI.</sub>
</p>
...
### G1 commanded locomotion benchmark

Example 67 wraps the same learned stride in a typed forward/stop/differential
steering command and evaluates the dynamic 23-DoF humanoid without a renderer.
The pinned full run checks transport, stopping, opposite signed left/right
steering paths, uprightness, a light pelvis-tilt disturbance, and exact replay
digests. Its `--train` path reproduces the seeded CEM over a contact-gated
differential torque overlay.
...
The current contact schedule validates signed path steering; measured body
heading remains nearly unchanged for the ±0.05&nbsp;rad/s commands. True
heading-yaw tracking is the next humanoid torque/contact milestone. See
[docs/G1_LOCOMOTION.md](docs/G1_LOCOMOTION.md) for the metrics and limitation.
