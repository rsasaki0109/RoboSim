...
cargo run --locked -p xtask -- release-bundle \
  ...
  --wheel artifacts/wheels/rne_py-0.4.0-*.whl \
  --python python
```

Use target `x86_64-pc-windows-msvc` on Windows. A tag build also passes
`--expected-tag v0.4.0`; the bundle reports `reproducible = true` only when the
...
cargo run --locked -p xtask -- release-install-smoke \
  --archive artifacts/release/rne-0.4.0-x86_64-unknown-linux-gnu.tar.gz \
  --bundle-dir artifacts/extracted/rne-0.4.0-x86_64-unknown-linux-gnu \
