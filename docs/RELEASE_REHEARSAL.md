...
`--expected-tag v0.3.0`; the bundle reports `reproducible = true` only when the
...
cargo run --locked -p xtask -- release-install-smoke \
  --archive artifacts/release/rne-0.3.0-x86_64-unknown-linux-gnu.tar.gz \
  --bundle-dir artifacts/extracted/rne-0.3.0-x86_64-unknown-linux-gnu \
