...
| CI and evaluation | `xtask parity`, capability and timing-free benchmark reports, portable Failure Capsules, stable hashes, robot/scenario replay artifacts, Behavior JSON/JUnit reports, and determinism contracts/tests | Partial | Stable evidence embeds exact/tolerance/outcome contracts, keeps wall-clock samples separate, verifies referenced replay hashes and schemas, and retains the M5 >=60 headless steps/s gate; broader frontend smoke remains |
