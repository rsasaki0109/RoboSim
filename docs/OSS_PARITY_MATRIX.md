...
| Physics selection and conformance | Rapier backend, analytic backend, capability negotiation, canonical snapshots, a deterministic unit-bearing conformance report, and a default-off MuJoCo 3.9 free-fall spike | Complete for the M4 Rapier/analytic slice; MuJoCo experimental | `xtask physics-conformance` proves every production capability with shared or capability-specific vectors; the MuJoCo spike advertises only rigid bodies and must pass its pinned-runtime exact-repeat test before promotion |
