...
| Physics selection and conformance | Rapier backend, analytic backend, capability negotiation, canonical snapshots, a deterministic unit-bearing conformance report, and a default-off MuJoCo 3.9 backend with rigid bodies, hinge/slide articulation, unit-explicit actuation, canonical contacts, and sensor overlaps | Complete for the M4 Rapier/analytic slice; MuJoCo experimental v0.3 slice | `xtask physics-conformance` proves every production capability with shared or capability-specific vectors; the feature-gated MuJoCo job proves its advertised rigid-body, articulation, and contact capabilities on pinned Windows/Linux runtimes |
...
| CI and evaluation | `xtask parity`, cross-platform `xtask evidence`, capability and timing-free benchmark reports, portable Failure Capsules, stable hashes, robot/scenario replay artifacts, Behavior JSON/JUnit reports, and determinism contracts/tests | Complete for the v0.2 trust slice | Linux and Windows run the aggregate evidence gate; stable evidence embeds exact/tolerance/outcome contracts, keeps wall-clock samples separate, verifies referenced replay hashes and schemas, and retains the M5 >=60 headless steps/s gate |
