...
| Scenario authoring | OpenSCENARIO 1.0 subset, parameters, catalogs, speed, lane change, assigned route, signals, controlled fixed-step execution, and versioned scenario replay artifacts | Parity for the current scenario slice | A scenario manifest writes a `rne-scenario-replay` JSON record and `rne-asset replay` re-executes the same XOSC/network/clock and compares the final result |
...
| Runner control and remote inspection | stdin/TCP `pause`, `resume`, `step`, `reset`, `quit`, live robot/traffic/sensor status snapshots, opt-in full-resolution RGB-D TCP payloads, versioned command transcripts, and scenario result artifacts | Parity for the current runner-control slice | Process-level TCP E2E verifies source-resolution RGB/depth dimensions and byte lengths, while controlled artifacts replay consumed commands including reset/quit |
