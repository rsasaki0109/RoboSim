...
`examples/63_g1_stride_gif`. Its blue panel is the plain hybrid stepper at the
forward command and its orange panel is `LEARNED_STRIDE` under the `+0.05
rad/s` differential-steering command; both panels use the same 8 s settle
