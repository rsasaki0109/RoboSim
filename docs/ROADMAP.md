...
fixture freezes their complete shard digest. The remaining evidence gap moved
from payload coverage to real RGB-D perception ground truth and completed
clean-run platform provenance.

The first real-scenario capture is now wired: example 73 records a successful
seeded diff-drive run with action, outcome, transform, IMU, and LiDAR streams.
Its 419-record manifest/shard summary is frozen and regenerated by the local
headless gate plus the Windows/Linux evidence matrix. The workflow definition
enforces cross-platform equality; a completed clean CI run is still required
before treating that equality as external platform evidence. RGB-D scenario
capture and evaluation against non-synthetic perception ground truth remain
open.
