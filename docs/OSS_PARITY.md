...
| Extension model | Dependency-free controller author ABI (`rne_plugin_sdk`), host-side manifests/interfaces (`rne_plugin`), dynamic loading through a versioned C ABI, offline scaffolding, standalone plugin conformance (`rne-asset plugin check`), and content-addressed external hardware-process conformance (`rne-hardware-conformance`) | Independent third-party plugin and backend/adapter certification remains an external v0.9 gate |
