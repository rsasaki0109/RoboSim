...
1. Add a minimal OpenSCENARIO/traffic adapter after the native run/replay
   ...
   route, and `${parameter}` references resolve from `ParameterDeclarations`.
   Wider OpenSCENARIO coverage (catalogs, route actions, network signals) is
   future work.
