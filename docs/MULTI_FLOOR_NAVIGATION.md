...
neither body can move, so the solver never resolves the pair. Example 120 drives
...
| --- | --- |
| 119 `elevator_ride` | A rider boards the car and rides 7 m across three floors with zero clearance drift; the car never travels with its doors open and never reports a boardable floor while parked elsewhere |
| 120 `elevator_call_button` | A dynamic fingertip presses the call button through solved contact forces; one press per contact however long it is held, and the car answers |
| 121 `multi_floor_mission` | A 1F-to-3F delivery planned through a `BuildingMap` and executed on the real elevator, routed onto the far lift because a wall seals the near lift's 2F landing |

Example 121's measured run: 5 legs at cost 60.25, 20.25 m driven, 22.65 s in
