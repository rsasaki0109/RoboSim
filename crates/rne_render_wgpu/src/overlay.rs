...
@fragment
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    let depth_m = textureSample(depth_tex, depth_sampler, input.uv).r;
    let normalized = clamp(1.0 - depth_m / max(params.max_depth_m, 0.0001), 0.0, 1.0);
...
/// Parameters for a single linear-depth overlay draw call.
pub struct DepthOverlayDraw<'a> {
    /// Command encoder for the overlay pass.
    pub encoder: &'a mut wgpu::CommandEncoder,
    /// Swapchain color target to composite onto.
    pub color_view: &'a wgpu::TextureView,
    /// Linear depth values in metres, row-major.
    pub depth_f32: &'a [f32],
    /// Image width in pixels.
    pub width: u32,
    /// Image height in pixels.
    pub height: u32,
    /// Far depth used to normalize the grayscale display.
    pub max_depth_m: f32,
    /// Bottom-left corner in NDC.
    pub ndc_min: [f32; 2],
    /// Top-right corner in NDC.
...
/// GPU texture overlay for renderer-independent linear-depth images.
...
impl DepthOverlay {
    /// Creates a linear-depth overlay renderer for the given surface format.
    pub fn new(device: &wgpu::Device, format: wgpu::TextureFormat) -> Self {
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("rne_depth_overlay"),
            source: wgpu::ShaderSource::Wgsl(DEPTH_SHADER.into()),
        });
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("rne_depth_overlay_layout"),
            entries: &[
                wgpu::BindGroupLayoutEntry {
                    binding: 0,
                    visibility: wgpu::ShaderStages::FRAGMENT,
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("rne_depth_overlay_pipeline_layout"),
            bind_group_layouts: &[&bind_group_layout],
            push_constant_ranges: &[],
        });
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("rne_depth_overlay_pipeline"),
            layout: Some(&pipeline_layout),
            vertex: wgpu::VertexState {
                module: &shader,
                entry_point: Some("vs_main"),
        ...
        let sampler = device.create_sampler(&wgpu::SamplerDescriptor {
            label: Some("rne_depth_overlay_sampler"),
            address_mode_u: wgpu::AddressMode::ClampToEdge,
            address_mode_v: wgpu::AddressMode::ClampToEdge,
            mag_filter: wgpu::FilterMode::Nearest,
            min_filter: wgpu::FilterMode::Nearest,
        ...
        let (texture, texture_view) = create_depth_texture(device, 1, 1);
        let uniform_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("rne_depth_overlay_uniform"),
            contents: bytemuck::bytes_of(&DepthParams {
                max_depth_m: 1.0,
                _padding: [0.0; 3],
            }),
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("rne_depth_overlay_bind_group"),
            layout: &bind_group_layout,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
        ...
        let vertex_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("rne_depth_overlay_vertices"),
            contents: bytemuck::cast_slice(&overlay_vertices(0.0, 0.0, 1.0, 1.0)),
            usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
        });
        Self {
    ...
    /// Uploads a linear-depth image into a GPU texture and draws it.
    pub fn draw(&mut self, device: &wgpu::Device, queue: &wgpu::Queue, draw: DepthOverlayDraw<'_>) {
        let DepthOverlayDraw {
            encoder,
            color_view,
            depth_f32,
            width,
        ...
        if width == 0 || height == 0 || depth_f32.len() < expected_len {
            return;
        }
        if width != self.texture_width || height != self.texture_height {
            let (texture, texture_view) = create_depth_texture(device, width, height);
            self.bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("rne_depth_overlay_bind_group"),
                layout: &self.bind_group_layout,
                entries: &[
        ...
        queue.write_texture(
            ...
            bytemuck::cast_slice(&depth_f32[..expected_len]),
            wgpu::TexelCopyBufferLayout {
                offset: 0,
                bytes_per_row: Some(4 * width),
                rows_per_image: Some(height),
        ...
        queue.write_buffer(
            &self.uniform_buffer,
            0,
            bytemuck::bytes_of(&DepthParams {
                max_depth_m: if max_depth_m.is_finite() && max_depth_m > 0.0 {
                    max_depth_m
                } else {
                    1.0
                },
        ...
        queue.write_buffer(&self.vertex_buffer, 0, bytemuck::cast_slice(&vertices));
        let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("rne_depth_overlay_pass"),
            color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                view: color_view,
                resolve_target: None,
                ops: wgpu::Operations {
...
fn create_depth_texture(
    ...
    let texture = device.create_texture(&wgpu::TextureDescriptor {
        label: Some("rne_depth_overlay_texture"),
        size: wgpu::Extent3d {
            width: width.max(1),
            height: height.max(1),
            depth_or_array_layers: 1,
