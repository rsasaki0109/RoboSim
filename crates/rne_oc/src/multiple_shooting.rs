...
mod tests {
    ...
    fn pendulum_model() -> rne_dynamics::ArticulatedModel {
        ...
        rne_dynamics::ArticulatedModel::from_robot(&world, robot).expect("model")
    }

    #[test]
    fn repairs_an_articulated_warm_start() {
        use crate::articulated::ArticulatedDynamics;
        let model = pendulum_model();
        let dynamics = ArticulatedDynamics::new(&model, 0.02);
        let horizon = 60;
        // An infeasible ramp warm start over the joint angle and velocity.
        let mut states = Vec::new();
        for k in 0..=horizon {
            let t = k as f64 / horizon as f64;
