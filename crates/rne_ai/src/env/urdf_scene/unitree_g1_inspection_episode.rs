...
impl Default for UnitreeG1InspectionEpisodeConfig {
    fn default() -> Self {
        Self {
            ...
            marker_name: "inspection_panel_check".into(),
            max_steps: 120,
        }
    }
}
...
impl UnitreeG1InspectionEpisode {
    ...
    pub fn new(config: UnitreeG1InspectionEpisodeConfig) -> Result<Self, AssetError> {
        ...
        settle(&mut sim);
        if sim.task_marker(&config.marker_name).is_none() {
            return Err(AssetError::Invalid {
                path: config.scene_path.display().to_string(),
                message: format!("missing task marker `{}`", config.marker_name),
            });
        }
        Ok(Self {
            config,
    ...
    fn observation(&self) -> UnitreeG1InspectionObservation {
        ...
        let (marker_x_m, _, marker_z_m, marker_radius_m) = self
            .sim
            .task_marker(&self.config.marker_name)
        ...
        UnitreeG1InspectionObservation {
            ...
            marker_distance_m,
            marker_radius_m,
            gesture_progress: ((self.step_in_episode.saturating_sub(60)) as f64 / 30.0)
                .clamp(0.0, 1.0),
...
impl Episode for UnitreeG1InspectionEpisode {
    ...
    fn step(&mut self, action: Self::Action) -> EpisodeStep<Self::Observation> {
        ...
        if action.advance {
            self.sim
                .step_joint_position_targets(&unitree_g1_inspection_targets(self.step_in_episode));
            self.step_in_episode += 1;
        }
        let observation = self.observation();
        let success = observation.inside_marker
            && observation.gesture_progress >= 1.0
            && self.step_in_episode >= INSPECTION_COMPLETE_STEP;
        let reward = 5.0 * (before_distance_m - observation.marker_distance_m)
...
mod tests {
    ...
    fn factory_inspection_completes_inside_named_marker() {
        ...
        let mut last = None;
        for _ in 0..INSPECTION_COMPLETE_STEP {
