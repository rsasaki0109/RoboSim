...
- `examples/112_go2_wbc_stance` whole-body-control diagnostic on a
  declared-inertial-mass Go2 scene, with new
  `assets/robots/unitree_go2_declared.rne.robot.toml` and its scene opting the
...
- `rne_dynamics::link_motions` was missing the frame-rotation term
  ...
  so the static tests and the earlier `qd` base-zero whole-body stance did not
  catch it, but it corrupts `LinkMotion::point_bias_acceleration_m_s2` and
  therefore the `rne_wbc` contact and CoM bias terms as soon as any link moves.
  The new finite-difference test pins the corrected conversion. Note: the
  previously reported "stable 240 Hz posture-only WBC stance" was an artifact of
  this bug; with the corrected bias the `rne_wbc` stance must be re-derived. See
