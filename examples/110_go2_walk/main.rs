//! Deterministic forward-trot walk for the official Unitree Go2.
//!
//! The scripted diagonal-pair trot is driven through position motors, the same
//! open-loop gait that the wider Go2 campaign uses. The example is headless by
//! default and asserts the measured boundary: forward transport, a roughly
//! straight path, and an upright body. Pass `--smoke` for the short CI gate or
//! `--gif` to capture `docs/media/unitree-go2-walk.gif` with wgpu (honoured only
//! when `RNE_SKIP_GPU` is unset).
//!
//! Run with `cargo run -p go2_walk --example 110_go2_walk`.

use std::fs;
use std::path::{Path, PathBuf};

use png::{BitDepth, ColorType, Encoder};
use rne_ai::{
    build_visual_render_scene, unitree_go2_dynamic_scene_path, unitree_go2_trot_targets,
    UnitreeGo2GaitCommand, UrdfSceneSim,
};
use rne_math::{Quat, Transform3, Vec3};
use rne_render::{
    Camera, MeshRenderCache, RenderBackend, RenderScene, RenderSceneItem, VisualShape,
};
use rne_render_wgpu::{CameraOrbit, WgpuRenderBackend};

const PANEL_WIDTH: u32 = 960;
const PANEL_HEIGHT: u32 = 540;
const FRAME_COUNT: usize = 96;
const STEPS_PER_FRAME: u64 = 5;
const SETTLE_STEPS: u64 = 120;
const SMOKE_STEPS: u64 = 240;
const FULL_STEPS: u64 = FRAME_COUNT as u64 * STEPS_PER_FRAME;
const CLEAR_COLOR: [f32; 4] = [0.03, 0.045, 0.07, 1.0];

/// Result of a headless trot rollout.
#[derive(Clone, Copy, Debug)]
struct WalkOutcome {
    displacement_x_m: f64,
    displacement_z_m: f64,
    min_height_m: f64,
    final_height_m: f64,
}

fn stand_command() -> UnitreeGo2GaitCommand {
    UnitreeGo2GaitCommand {
        stride_rad: 0.0,
        foot_lift_rad: 0.0,
        cycle_steps: 90,
        ..UnitreeGo2GaitCommand::default()
    }
}

fn settled_sim() -> UrdfSceneSim {
    let mut sim =
        UrdfSceneSim::from_scene_path(&unitree_go2_dynamic_scene_path()).expect("load dynamic Go2");
    sim.configure_position_motors(180.0, 18.0, 23.7);
    let stand = unitree_go2_trot_targets(0, stand_command());
    for _ in 0..SETTLE_STEPS {
        sim.step_joint_position_targets(&stand);
    }
    sim
}

fn walk(sim: &mut UrdfSceneSim, steps: u64) -> WalkOutcome {
    let start = sim.observe();
    let mut min_height_m = f64::MAX;
    for step in 0..steps {
        sim.step_joint_position_targets(&unitree_go2_trot_targets(
            step,
            UnitreeGo2GaitCommand::default(),
        ));
        min_height_m = min_height_m.min(sim.observe().base_y_m);
    }
    let end = sim.observe();
    WalkOutcome {
        displacement_x_m: end.base_x_m - start.base_x_m,
        displacement_z_m: end.base_z_m - start.base_z_m,
        min_height_m,
        final_height_m: end.base_y_m,
    }
}

fn assert_walk(outcome: WalkOutcome, min_forward_m: f64) {
    assert!(outcome.final_height_m > 0.18, "Go2 trot fell: {outcome:?}");
    assert!(
        outcome.min_height_m > 0.15,
        "Go2 trot dropped too low: {outcome:?}"
    );
    assert!(
        outcome.displacement_x_m.abs() > min_forward_m,
        "Go2 trot must cover ground: {outcome:?}"
    );
    assert!(
        outcome.displacement_z_m.abs() < 0.25,
        "Go2 trot must stay roughly straight: {outcome:?}"
    );
}

fn main() {
    if std::env::args().any(|argument| argument == "--smoke") {
        let mut sim = settled_sim();
        let outcome = walk(&mut sim, SMOKE_STEPS);
        assert_walk(outcome, 0.08);
        println!("go2 walk smoke passed: {outcome:?}");
        return;
    }
    if !std::env::args().any(|argument| argument == "--gif") {
        let mut sim = settled_sim();
        let outcome = walk(&mut sim, FULL_STEPS);
        assert_walk(outcome, 0.35);
        println!("go2 walk passed: {outcome:?}");
        return;
    }
    if std::env::var("RNE_SKIP_GPU").is_ok() {
        return;
    }

    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
    let media_dir = repo_root.join("docs/media");
    let frames_dir = media_dir.join("unitree-go2-walk-frames");
    let _ = fs::remove_dir_all(&frames_dir);
    fs::create_dir_all(&frames_dir).expect("create Go2 walk frame directory");

    let mut sim = settled_sim();
    let start_x_m = sim.observe().base_x_m;
    let mut backend = WgpuRenderBackend::new().expect("initialize wgpu");
    let camera = Camera::new(PANEL_WIDTH, PANEL_HEIGHT, std::f64::consts::FRAC_PI_4);
    let mesh_roots: Vec<PathBuf> = sim.mesh_package_roots().to_vec();
    let mesh_root_refs: Vec<&Path> = mesh_roots.iter().map(PathBuf::as_path).collect();
    let mut mesh_cache = MeshRenderCache::new();

    let mut min_height_m = f64::MAX;
    for frame in 0..FRAME_COUNT {
        for _ in 0..STEPS_PER_FRAME {
            let step = frame as u64 * STEPS_PER_FRAME;
            sim.step_joint_position_targets(&unitree_go2_trot_targets(
                step,
                UnitreeGo2GaitCommand::default(),
            ));
        }
        min_height_m = min_height_m.min(sim.observe().base_y_m);
        let rgba = render_panel(
            &mut backend,
            &camera,
            &mut mesh_cache,
            &mesh_root_refs,
            &sim,
        );
        write_png(
            &frames_dir.join(format!("frame-{frame:03}.png")),
            &rgba,
            PANEL_WIDTH,
            PANEL_HEIGHT,
        )
        .expect("write Go2 walk frame");
    }

    let end = sim.observe();
    let forward_m = end.base_x_m - start_x_m;
    assert!(end.base_y_m > 0.18, "Go2 walk fell while capturing");
    assert!(
        min_height_m > 0.15,
        "Go2 walk dropped too low while capturing"
    );
    assert!(
        forward_m.abs() > 0.35,
        "Go2 walk must cover ground: {forward_m:.3} m"
    );

    let gif_path = media_dir.join("unitree-go2-walk.gif");
    build_gif(&frames_dir, &gif_path).expect("encode Go2 walk gif");
    image::open(frames_dir.join(format!("frame-{:03}.png", FRAME_COUNT - 1)))
        .expect("read Go2 walk poster frame")
        .save(media_dir.join("unitree-go2-walk.png"))
        .expect("write Go2 walk poster");
    let _ = fs::remove_dir_all(&frames_dir);
    println!(
        "rendered Go2 walk media to {} (forward {:.3} m, minH {:.3} m)",
        gif_path.display(),
        forward_m,
        min_height_m,
    );
}

fn render_panel(
    backend: &mut WgpuRenderBackend,
    camera: &Camera,
    mesh_cache: &mut MeshRenderCache,
    mesh_root_refs: &[&Path],
    sim: &UrdfSceneSim,
) -> Vec<u8> {
    let observed = sim.observe();
    let mut scene = build_visual_render_scene(sim.world());
    scene
        .items
        .retain(|item| !matches!(item.shape, VisualShape::Box { .. }));
    append_checker_floor(&mut scene, observed.base_x_m, observed.base_z_m, 0.12);
    mesh_cache
        .resolve_scene(&mut scene, mesh_root_refs)
        .expect("resolve official Go2 meshes");
    let orbit = CameraOrbit {
        focus: Vec3::new(
            observed.base_x_m,
            observed.base_y_m + 0.04,
            observed.base_z_m,
        ),
        yaw_rad: -1.6,
        pitch_rad: 1.35,
        distance_m: 1.6,
    };
    let output = backend
        .render_scene_camera(camera, &orbit.camera_transform(), &scene, CLEAR_COLOR)
        .expect("render Go2 walk panel");
    output.color.rgba8
}

fn append_checker_floor(scene: &mut RenderScene, center_x_m: f64, center_z_m: f64, tile_m: f64) {
    let snap = |value: f64| (value / (2.0 * tile_m)).floor() * 2.0 * tile_m;
    for row in -8..=8 {
        for column in -8..=8 {
            let color = if (row + column) & 1 == 0 {
                [0.11, 0.15, 0.21, 1.0]
            } else {
                [0.055, 0.075, 0.11, 1.0]
            };
            scene.items.push(RenderSceneItem {
                transform: Transform3 {
                    translation: Vec3::new(
                        snap(center_x_m) + column as f64 * tile_m,
                        -0.008,
                        snap(center_z_m) + row as f64 * tile_m,
                    ),
                    rotation: Quat::IDENTITY,
                    scale: Vec3::new(tile_m * 0.96, 0.008, tile_m * 0.96),
                },
                shape: VisualShape::Box { size_m: Vec3::ONE },
                color_rgba: color,
                mesh: None,
                base_color_texture: None,
                material: Default::default(),
            });
        }
    }
}

fn build_gif(frames_dir: &Path, gif_path: &Path) -> std::io::Result<()> {
    let status = std::process::Command::new("ffmpeg")
        .args([
            "-y",
            "-framerate",
            "12",
            "-i",
            &frames_dir.join("frame-%03d.png").to_string_lossy(),
            "-vf",
            "fps=12,scale=720:-1:flags=lanczos,split[s0][s1];[s0]palettegen=max_colors=160[p];[s1][p]paletteuse=dither=bayer:bayer_scale=3",
            &gif_path.to_string_lossy(),
        ])
        .status()?;
    status
        .success()
        .then_some(())
        .ok_or_else(|| std::io::Error::other("ffmpeg Go2 walk gif encode failed"))
}

fn write_png(path: &Path, rgba: &[u8], width: u32, height: u32) -> std::io::Result<()> {
    let file = fs::File::create(path)?;
    let mut encoder = Encoder::new(file, width, height);
    encoder.set_color(ColorType::Rgba);
    encoder.set_depth(BitDepth::Eight);
    let mut writer = encoder.write_header()?;
    writer.write_image_data(rgba).map_err(std::io::Error::other)
}
