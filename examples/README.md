...
| Headless scene runner | `cargo run --release -p rne_asset_cli -- simulate assets/scenes/mesh_diff_drive.rne.scene.toml --steps 600 --hz 60 --wheel-velocity-rad-s 6 --determinism-check` | Fixed-step scene execution, typed wheel command, final physics hash, and exact replay check |
| Run manifest | `cargo run --release -p rne_asset_cli -- run assets/runs/mesh_diff_drive.rne.run.toml` | Versioned scene, seed, clock, controller, and determinism configuration |
