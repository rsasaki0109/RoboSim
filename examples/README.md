...
| Unitree G1 photoreal capture | `cargo run -p g1_photoreal_capture --example 70_g1_photoreal_capture -- --smoke` | Resolves the official G1 URDF/STL visual hierarchy headlessly; the GPU path adds PBR test-bay materials, optional HDRI/TAA, and PNG/GIF capture |
| Unitree G1 RGB-D sensor capture | `cargo run -p g1_rgbd_sensor --example 71_g1_rgbd_sensor -- --smoke` | Mounts a renderer-independent RGB-D camera on `head_link`, validates DataBus latency and deterministic hashes headlessly, and writes GPU RGB/depth captures plus a manifest |
