...
| G1 learned stride | `cargo run --release -p g1_learned_stride --example 62_g1_learned_stride` | Verifies the humanoid's first real steps: a learned torque overlay turns the near-stationary scripted stepper into a slow genuine walk (over 2x the stepper's transport) at full height and dead straight; `-- --train` reproduces the search |
