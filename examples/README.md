...
| Go2 torque-space turn search | `cargo run --release -p go2_torque_turn --example 56_go2_torque_turn` | Searches contact-gated feed-forward torques on the torque-PD walk and verifies the pinned plateau break: 0.038 rad/s in the chaos-robust window, above every position-space result; `-- --train` reproduces the search, `-- --ensemble` the chaos-horizon spread |
