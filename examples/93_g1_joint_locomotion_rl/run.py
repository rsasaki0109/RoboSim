"""Gymnasium-style Unitree G1 joint-space locomotion wrapper and headless smoke.

The native `rne_py.UnitreeG1JointLocomotionEpisode` owns the physics,
observation, reward, seed, and termination. This file only adapts it to the
Gymnasium API. The observation/action spaces are derived from the native
TaskSpec so there is no second handwritten space declaration.
"""
...
class UnitreeG1JointLocomotionEnv(_Base):
    """RL environment backed by the native deterministic G1 joint episode."""
    ...
    def __init__(
        ...
        self._command_forward_m_s = command_forward_m_s
        self._command_yaw_rate_rad_s = command_yaw_rate_rad_s
        self._episode = rne_py.UnitreeG1JointLocomotionEpisode(
            max_steps,
            seed,
            command_forward_m_s,
    ...
    def _wrap_observation(self, observation):
        if len(observation) != self.observation_dim:
            raise RuntimeError(f"unexpected G1 observation length: {len(observation)}")
        if not all(math.isfinite(value) for value in observation):
            raise RuntimeError("G1 observation contains a non-finite value")
        if _HAS_GYM:
    ...
    def step(self, action):
        ...
        result = self._episode.step([float(value) for value in values])
        return (
            ...
            bool(result.truncated),
            {
                ...
                "task_id": self.task_spec["task_id"],
                "base_x_m": self._episode.base_x_m,
                "base_y_m": self._episode.base_y_m,
            },
        )


def evaluate_forward_transport(env, policy=None, steps=300):
    """Rolls one deterministic episode and returns (displacement, upright)."""
    observation, _ = env.reset()
    start_x = env._episode.base_x_m
    min_height = env._episode.base_y_m
    for _ in range(steps):
        ...
        min_height = min(min_height, info["base_y_m"])
        if terminated or truncated:
            break
    return env._episode.base_x_m - start_x, min_height


def main():
    env = UnitreeG1JointLocomotionEnv(max_steps=64, seed=9301, command_forward_m_s=0.4)
    observation, _ = env.reset()
    action = [0.0] * env.action_dim
    total_reward = 0.0
    steps = 0
    ...
    if "--smoke" in sys.argv:
        if len(observation) != env.observation_dim or not math.isfinite(total_reward):
            raise SystemExit("G1 joint locomotion RL smoke failed: invalid rollout")
        backend = "gymnasium" if _HAS_GYM else "list-fallback"
        print(
            "G1 joint locomotion RL smoke ok: "
            f"obs={env.observation_dim} act={env.action_dim} "
            f"reward={total_reward:.3f} steps={steps} backend={backend}"
        )
        return
    print(f"G1 joint locomotion rollout: reward={total_reward:.3f} steps={steps}")
