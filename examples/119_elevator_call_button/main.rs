...
//!
//! ```text
//! cargo run --release -p elevator_call_button --example 119_elevator_call_button
//! ```
...
/// Spawns the fixed button body and the dynamic fingertip that presses it.
fn spawn_panel(world: &mut World) -> (rne_ecs::Entity, rne_ecs::Entity) {
    ...
    let button_body = spawn_named(world, "call_button");
    ...
    let presser = spawn_named(world, "presser");
...
/// Builds the button device with the thresholds the presser is sized for.
fn call_button() -> CallButton {
    CallButton::new(CallButtonSpec {
...
fn main() {
    ...
    let (button_body, presser) = spawn_panel(&mut world);
    let mut button = call_button();
