//! Learns a commanded yaw-rate tracking torque policy for the Go2.
//!
//! The reference-free state-feedback policy (`57_go2_torque_policy`) sustains
//! a turn while walking, but Linux CI showed its turn *direction* is
//! chaos-selected: feedback without a reference shapes dynamics, it does not
//! encode a command. This example adds the missing rung — the yaw-rate
//! feature becomes an *error term* against a commanded reference, and the CEM
//! objective scores each candidate by the **worse of its two commanded
//! directions**, so no chaos-selected one-way orbit can win: the winner must
...
/// Assembles the policy's feature vector. Identical to example 57 except
/// feature 4: the raw yaw rate becomes the tracking error against the
/// commanded reference — the term that turns dynamics-shaping into steering.
fn features(
    sim: &UrdfSceneSim,
    step: u64,
    cycle: u64,
...
fn rollout(policy: &UnitreeGo2TorquePolicy, yaw_rate_ref_rad_s: f64, steps: u64) -> RolloutOutcome {
    ...
    for step in 0..steps {
        ...
        let feed_forward = policy.torques_nm(&features(&sim, step, cycle, yaw_rate_ref_rad_s));
...
/// Commanded score: the WORSE of the two commanded directions. A
/// chaos-selected one-way orbit scores its bad direction, so only policies
/// that genuinely obey the command survive — the two commands double as a
/// diverse ensemble against single-trajectory objective gaming.
...
fn state_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../target/go2_steered_cem_state.txt")
}
...
fn main() {
    ...
    for (reference, outcome) in [
        ...
        println!(
            "commanded ref {reference:+.2}: windows {:+.3}/{:+.3}, fwd {:.2} m, tilt {:.2}",
            outcome.window_a_yaw_rad,
            outcome.window_b_yaw_rad,
            outcome.forward_m,
            outcome.max_tilt_rad
